...
fn load_module(module: &str) -> Result<(), ()> {
    print_info(&format!("Loading module {}...", module));
    ...
    match status {
        ...
        Ok(_) | Err(_) => {
            print_error(&format!("Failed to load module: {}", module));
            Err(())
        }
    }
}
...
pub fn load_modules_from_file(
    ...
    print_info(&format!("Reading modules from {}", path));
    io::stdout().flush().unwrap();
    ...
    for line in contents.lines() {
        ...
        match (requirement, result) {
            ...
            (_, Err(_)) => {
                print_info(&format!("Optional module {} failed to load, continuing.", module));
            }
            (_, Ok(())) => {}
        }
    }
